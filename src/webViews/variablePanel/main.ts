...
window.addEventListener(
  ...
  ({
    data,
    origin,
  }: MessageEvent<DhVariablePanelMsg | VscodeVariablePanelMsg>) => {
    ...
    // Forward error notifications from DH iframe -> VS Code
    if (isErrorNotificationFromDh(data)) {
      logger.info('Received error notification from iframe');
      vscode.postMessage({ data });
      return;
    }
